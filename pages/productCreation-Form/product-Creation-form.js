...
function populateCategoryDropdown(categories) {
  ...
  while (select.options.length > 2) {
    // assuming first 2 options are 'Select Category' and 'Add New'
    select.remove(2);
  }
  ...
  try {
    const response = await fetch(API_URL + "/products/addProduct", {
      ...
      headers: {
        ...
        // Note: Content-Type header is not set as it's automatically handled with FormData
      },
      body: formData,
    });
    ...
    if (response.ok) {
      ...
      window.router.navigate("/"); // Redirect or handle successful submission as needed
    } else {
